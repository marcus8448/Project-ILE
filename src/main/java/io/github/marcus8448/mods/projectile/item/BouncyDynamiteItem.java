...
public class BouncyDynamiteItem extends Item {

    public BouncyDynamiteItem(Properties properties, ResourceLocation id) {
        super(properties);
        this.setRegistryName(id);

        DispenserBlock.registerDispenseBehavior(this, new ProjectileDispenseBehavior() {
            protected IProjectile getProjectileEntity(World worldIn, IPosition position, ItemStack stackIn) {
                return Util.make(new BouncyDynamiteEntity(worldIn, position.getX(), position.getY(), position.getZ()), (p_218408_1_) -> p_218408_1_.func_213884_b(stackIn));
            }
        });
    }
    ...
    public ActionResult<ItemStack> onItemRightClick(World world, PlayerEntity player, Hand hand) {
        ...
        if (!world.isRemote) {
            BouncyDynamiteEntity snowball = new BouncyDynamiteEntity(world, player);
